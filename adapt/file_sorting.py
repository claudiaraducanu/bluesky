...
def move_log_files():
    ...
    for l in set(log_type):
        for id in set(acid):
            for date in set(creation_date):
                dir_name = os.path.join(root,"adapt","outputBlueSky",l,id,date)
    ...
    dst_path = [os.path.join(root,"adapt","outputBlueSky",log_type[idx],acid[idx],creation_date[idx])
...
def log_to_pkl_arc():
    ...
    root = os.getcwd()
    simData = os.path.join(root,"outputBlueSky")
