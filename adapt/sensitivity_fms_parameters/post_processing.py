...
assigned_wp = pkl.load(open('assigned_rta_waypoints.pkl','rb'))
assigned_wp = pd.DataFrame(assigned_wp,columns=["flightid","rta_nwp",'rta_wp'])
assigned_wp  = assigned_wp.set_index("flightid")

flights_with_missed_wp = []
...
for i in range(ndir):
    ...
    for file in ifiles:
        ...
        # way-points with rta assigned to follow
        if not list(waypoints_rta['actrtawp']) == assigned_wp.loc[flightid].rta_wp:
            print(idir)
            flights_with_missed_wp.append([idir,flightid,list(set(assigned_wp.loc[flightid].rta_wp) -
                                                         set(waypoints_rta['actrtawp']))])
