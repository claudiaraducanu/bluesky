...
import pandas as pd
from adapt.scripts.download_forecast import DownloadWind
import numpy as np
...
if __name__ == "__main__":
    ...
    data_set_path = os.path.join(os.getcwd(),"ddr2")
    sim_scn_path  = os.path.join(bluesky_scn_path,"symmetric",datetime.datetime.now().strftime("%Y%m%d"),"individual")
    ...
    # Import the list of trajectories that have a symmetric time window
    constrain = []
    trajectory_list          = pd.read_excel(os.path.join(os.getcwd(),"xlsx","symmetric.xlsx"))
    trajectory_list          = trajectory_list.set_index("acid")

    trajectory_list["01"]                   = 1
    trajectory_list["60"]                   = 60
    trajectory_list["15"]                   = trajectory_list["opening"] + trajectory_list["closing"]
    ...
    coefficient.init(os.path.join("/Users/Claudia/Documents/5-MSc-2/bluesky", "data/performance/BADA"))
    day_before_departure = 1

    """ Loop through the DDR trajectories to create scn files"""

    tw_duration      = ["01","60","15"]
    departure_delay  = [0,5,10,15,20]

    """ Configure the paths for the wind data"""

    netcdf_path = os.path.join(os.getcwd(),'netcdf')
    grib_path   = os.path.join(os.getcwd(), "grib")
